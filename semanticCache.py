...
def get_embedding(query):
    ...
    st.write(f"Generating embedding for query: {query}")
    model = load_model()
...
def search_similar_vectors(query_embedding):
    ...
    st.write("Searching for similar vectors in the vector store...")
    similar_vectors = []
    for stored_query, stored_embedding in st.session_state.vector_store.items():
        ...
        st.write(f"Similarity for query '{stored_query}': {similarity}")
        if similarity > similarity_threshold:
            similar_vectors.append((stored_query, similarity))
            st.write(f"Added '{stored_query}' to similar vectors with similarity {similarity}")
    st.write(f"Found {len(similar_vectors)} similar vectors")
    return similar_vectors

def add_to_vector_store(query, response):
    """Add a query and its response to the vector store."""
    st.write(f"Adding query '{query}' and response '{response}' to the vector store...")
    query_embedding = get_embedding(query)
    st.session_state.vector_store[query] = query_embedding
    st.session_state.vector_store[f"{query}_response"] = response
    st.write(f"Added query '{query}' with embedding {query_embedding} and response '{response}'")

def get_response_from_cache(query):
    """Get a response from the cache if a similar query exists."""
    st.write(f"Checking cache for query '{query}'...")
    query_embedding = get_embedding(query)
    if query in st.session_state.vector_store:
        st.write(f"Exact query '{query}' found in cache")
        return st.session_state.vector_store[f"{query}_response"]
    
    similar_vectors = search_similar_vectors(query_embedding)
    if similar_vectors:
        st.write(f"Found similar vectors for query '{query}'")
        top_query = max(similar_vectors, key=lambda x: x[1])[0]
        st.write(f"Using response from top similar query '{top_query}'")
        return st.session_state.vector_store[f"{top_query}_response"]
    else:
        st.write(f"No similar queries found for '{query}'")
        return None

def call_llm(query):
    """Call OpenAI API to get a response."""
    st.write(f"Calling LLM for query '{query}'...")
    try:
        ...
        st.write(f"Received response from LLM: {response.choices[0].message.content}")
        return response.choices[0].message.content
    except Exception as e:
        st.write(f"Error calling LLM: {e}")
...
def main():
    ...
    if st.button("Submit"):
        ...
        if cached_response and not cached_response.startswith("Error calling LLM:"):
            st.write(f"Using cached response for query '{query}'")
            st.write(f"Response from cache: {cached_response}")
        else:
            try:
                llm_response = call_llm(query)
                add_to_vector_store(query, llm_response)
                st.write(f"Added query '{query}' and response '{llm_response}' to cache")
                st.write(f"Response from LLM: {llm_response}")
            except Exception as e:
                st.write(f"Error processing query '{query}': {e}")
